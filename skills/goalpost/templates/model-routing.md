...
| `claude:opus` | Session model (`model: inherit` — Fable 5.1 on a Fable 5.1 host; Claude Opus 5 until 2026-09-03) | Claude subagent (`goalpost:goal-worker-scoped`) | **Scoped implementation & edits** — a well-specified change to named files with an executable DoD (engineering included, hybrid lane), or a routine creative variant. NOT 0→1 design/architecture/research (those stay `fable`) | `medium` | session-model rate at medium effort (Fable 5.1 medium ≈ Fable 5 at lower cost per task; beats Opus/Sonnet on cost per task at low/medium — Anthropic Fable 5.1 prompting guide) |
...
## Claude-side routing — the scoped-implementation lane (owner decision, 2026-07-26)

FrontierCode 1.1 measured Claude Opus 5 peaking at **medium** effort on mergeability (Main 53.4% at medium vs 48.0% at high — above medium, the model loses points to out-of-scope refactors hitting the scope blocker), which matches goalpost's own scope discipline. Opus 5 medium was the 2026-07-26 choice; on **2026-09-03 the owner moved the scoped worker to the session model** at medium (`model: inherit` — Fable 5.1 medium on a Fable 5.1 host, which per Anthropic's Fable 5.1 prompting guide roughly matches Fable 5 at lower cost and beats Opus/Sonnet on cost per task at low/medium). The medium-effort finding carries over; the model pin does not. The owner adopted a hybrid split:

- **`claude:fable`** — judgement work: planning, stage/goal design, 0→1 design/architecture/research, adversarial review, creative taste. **Never effort-downshift this lane** — the medium-beats-high result is specific to scoped PR-style edits and does NOT generalize to open-ended design work.
- **`claude:opus/medium`** — scoped implementation & edits: a well-specified change to named files with a clear executable DoD, or a routine creative variant. Dispatches to the `goalpost:goal-worker-scoped` agent (frontmatter `model: inherit`, `effort: medium` — the session model at medium), whose contract forbids out-of-scope refactors.
- **Hybrid boundary vs Codex:** Codex keeps the parallel-capacity role (Codex works while Claude orchestrates other goals) and the cross-vendor verification role (a different vendor's review catches shared blind spots). Important scoped implementations MAY route `[claude:opus/medium]`; parallel fan-out legs and independent cross-checks stay `[codex:*]`.
- **Effort mechanics (Claude side):** per-goal effort rides the worker agent's **frontmatter** (`effort:`), never a mid-session change (changing effort mid-conversation invalidates the prompt cache). The reviewer lane (`goalpost:transition-reviewer`) runs `effort: high` — verification is a final gate, not a scoped edit.
...
- **Model**: append `:<tier>` — `sol` / `terra` / `luna` for codex; `fable` / `opus` for claude. Claude-side tier names are **routing keys, not model pins**: `opus` selects the `goalpost:goal-worker-scoped` lane (session model via `model: inherit`, medium effort), so `[claude:opus/medium]` in an existing ledger still routes there unchanged.
...
- **Optional effort suffix**: `[codex:luna/xhigh]`, `[codex:sol/ultra]`, `[codex:terra/high]`, `[claude:opus/medium]`. Omit to use the roster's default effort. Passed to Codex as `config: { model_reasoning_effort: "<effort>" }`; on a `claude` tag the effort is realized by the worker agent's frontmatter (`goal-worker-scoped` = `medium`), not an API param. On an up-escalation the new tier uses its own default effort (a `luna/xhigh` that escalates becomes `terra/high`, then `sol/ultra`) — the suffix pins the *starting* effort, not the ladder.
...
- "Apply the reviewed two-file fix exactly as specified (named files, named test)" → scoped implementation → `claude:opus/medium` (hybrid lane), or `codex:terra` when Codex parallel capacity / cross-vendor separation is preferred.
...
| **Quota exhaustion** (429/usage cap on the assigned platform) | no | substitute worker on the OTHER platform, **same DoD** | auto-substitute + record `[<orig>→<sub>(폴백 — 사유·실측)]` on the row (owner decision 2026-08-02, elonfeedback `goalpost/2026-07-31/010`); a `sol`-required goal is NEVER substituted down → `HUMAN_GATE(model-unavailable)` |
...
- **Up-escalation to Sol is pre-authorized — do not stop to ask** (owner decision 2026-08-01, elonfeedback `goalpost/2026-07-31/003`: "최고급까지 자동 상향, 안전레인 필수"). A quality-strike escalation that reaches `sol` needs no per-goal owner approval; a ledger-local rule like "sol은 오너 승인 후에만" is superseded by this standing decision. The non-negotiable counterpart: an escalated Sol attempt ALWAYS runs on the gated lane (`workspace-write` + the guardrail block, never `danger-full-access`), and the SAFETY_STOP row below still never escalates.
...
- **Claude-side ladder** (creative goals): `opus` → `fable`. The `fable` attempt runs as a **fresh reviewer/author subagent**, never inline in the orchestrator (writer/judge separation, principle 4). A creative strike escalates the author tier the same way, and the fresh-context C-row scoring is unchanged. The same ladder applies to the scoped-implementation lane: a `claude:opus/medium` strike escalates to `fable` (fresh subagent, default effort) — a scoped goal that failed at medium is evidence it was not actually scoped/settled, so the retry gets the judgement tier.
...
1. **Min-tier floor** — a bare `[codex]`/`[claude]` tag, a goal with **no routing tag at all**, or a **tag not in the roster** (e.g. `[claude:terra]`, prose executor notes, ad-hoc fallback notations) is **unclassified**: classify it now by blast radius × variance and **write the resolved tier back onto the goal's ledger row** before dispatch, so the classification is a recorded fact rather than a per-session guess (owner decision 2026-08-01, elonfeedback `goalpost/2026-07-31/002` — pre-dispatch auto-classification is mandatory, the last safety net for goals the plan-time tagging missed). Never silently default a security/credential/payment/irreversible-migration goal to `terra`. An explicit roster tag is still subject to the HIGH-radius → Sol floor.
