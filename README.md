...
![version](https://img.shields.io/badge/version-0.4.2-2563eb) ![license](https://img.shields.io/badge/license-MIT-16a34a) ![platform](https://img.shields.io/badge/Claude%20Code-plugin-7c3aed) &nbsp; **Install:** `/plugin install goalpost@goalpost`
...
| `opus` | Session model (`model: inherit` — Fable 5.1 on a Fable 5.1 host) | **Scoped implementation & edits** (hybrid lane) — well-specified changes to named files, routine creative variants. Runs via `goalpost:goal-worker-scoped`. | `medium` |

Pick the tier by **blast radius × variance**, route on the stronger signal (LOW+LOW → Luna; MED → Terra; HIGH → Sol), and **never downgrade a security/data/payment/migration goal to save cost**. A DoD strike **escalates one tier up** (Luna→Terra→Sol) rather than repeating the same tier; the 3-strike cap is unchanged. Independent, disjoint-file goals — and `[fanout]` goals with a per-sub-task tier map — run concurrently at right-sized tiers via a single Workflow level. On the Claude side, `fable` handles judgement/0→1 work and the `opus` tag routes scoped implementation & edits to `goalpost:goal-worker-scoped`, which runs the **session model at `medium` effort** (`model: inherit` — Fable 5.1 medium on a Fable 5.1 host; owner decision 2026-09-03; the tag is a routing key, not a model pin). Medium effort is grounded in FrontierCode 1.1, where Claude Opus 5 — the 2026-07-26 choice — peaks at medium on scoped PR-style work and loses points above it to out-of-scope refactors; Codex keeps the parallel-capacity and cross-vendor-verification roles (hybrid, owner decision 2026-07-26).
