...
| `claude:opus` | Claude Opus 5 | Claude subagent (`goalpost:goal-worker-scoped`) | **Scoped implementation & edits** — a well-specified change to named files with an executable DoD (engineering included, hybrid lane), or a routine creative variant. NOT 0→1 design/architecture/research (those stay `fable`) | `medium` | lower than Fable (~$5/$25 per 1M) |
...
FrontierCode 1.1 measured Claude Opus 5 peaking at **medium** effort on mergeability (Main 53.4% at medium vs 48.0% at high — above medium, the model loses points to out-of-scope refactors hitting the scope blocker), which matches goalpost's own scope discipline. The owner adopted a hybrid split:
...
- **`claude:opus/medium`** — scoped implementation & edits: a well-specified change to named files with a clear executable DoD, or a routine creative variant. Dispatches to the `goalpost:goal-worker-scoped` agent (frontmatter `model: opus`, `effort: medium`), whose contract forbids out-of-scope refactors.
