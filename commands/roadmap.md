...
3. Decompose into 5–10 dependency-ordered stages, then 8–12 **atomic** goals per stage — each with a routing tag (`[codex]`/`[claude]`/`[mixed]`), a `[depends]` list, and a DoD: `[codex]` goals get an **executable** DoD (no executable check → the goal is too big, split it); `[claude]` goals get the **checklist** DoD (production-readiness C-rows, judged by a fresh reviewer) and are NOT split for lacking an executable check.
