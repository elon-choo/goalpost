...
### Startup
1. **Fix the target repo, then select the active ledger from inside it only.** Target repo = the user-named path, else `git rev-parse --show-toplevel` (else cwd). Find every goalpost-generated `LEDGER*.md` (`Generator: goalpost` header) **under that repo only** — shared document folders, other projects, recall, and memory are never searched for ledgers (principle 8). Ignore ledgers without the goalpost generator marker (they belong to another tool — do not drain them under this contract). **If the repo has no ledger, stop and ask the user** (start a PLAN here vs. name the intended project) — never widen the search. Pick the active one by: (a) `Created:` header, newest → tie-break (b) filename timestamp → tie-break (c) file mtime. **Integrity check the chosen ledger** before using it: it must contain `▶ NEXT:`, a status-codes section, at least one goal row, **and a `Target repo:` header that matches the current target repo** (mismatch → stop and ask; it may be a mirror copy from another project); if it looks truncated (e.g. a mid-write crash), do not resume — report it and ask. Declare the chosen path + completion % + stop conditions in one line. If the newest is already fully `[x]`, report "newest ledger complete" and confirm before resuming an older one or starting a new PLAN.
